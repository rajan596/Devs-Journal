...
- Here we will need to put a lock before actual schedule method call.
- I have used Reader writer lock to manage Reads for history and writes for actual booking process.
    - If read is not important then only ReentrantLock or Semaphore can also work.
- Updated Code
    - MeetingRoom::bookMeeting
    - MeetingRoom::showMeetingHistory
    - Added Fixes thread executor in Driver class to have 3 threads trying to book and acquire lock
...
public class MeetingScheduler {
    ...
    public MeetingRoom scheduleMeeting(Meeting meeting){
        ...
        if(meetingRoom != null) {
            meetingRoom.bookMeeting(meeting);
            meeting.setMeetingRoom(meetingRoom);
            System.out.println(String.format("[%s] Meeting Scheduled for %s with MeetingRoom: %s", Thread.currentThread().getName() , meeting.meetingTitle, meeting.meetingRoom.roomId));
            notificationService.sendNotification(meeting.getParticipants(),"You have been invited for meeting - " + meeting.getMeetingTitle());
        } else {
            System.out.println("Unable to schedule meeting for : " + meeting.meetingTitle);
        }
...
class MeetingRoom {
    ...
    public void bookMeeting(Meeting meeting) {
        try{
            System.out.println(String.format("\t[%s] Trying to get lock for roomId:%s", Thread.currentThread().getName(), roomId));
            lock.writeLock().lock();
            System.out.println(String.format("\t[%s] Lock Acquired for roomId:%s", Thread.currentThread().getName(), roomId));
            this.calender.bookSlot(meeting);
            if(this.meetingHistory.size() >= MAX_HISTORY_COUNT) {
                this.meetingHistory.remove();
            }
            ...
            System.out.println(String.format("\t[%s] Released for roomId:%s", Thread.currentThread().getName(), roomId));
        }
    }
...
class DriverClass {
    public static void main(String[] args) {
        ...
        List<Meeting> meetings = List.of(
               new Meeting("meeting1",1,10, new User("1"), List.of(new User("20"))),
               new Meeting("meeting2",1,10, new User("2"), List.of(new User("20"))),
               new Meeting("meeting3",1,10, new User("3"), List.of(new User("20"))),
               new Meeting("meeting4",1,10, new User("4"), List.of(new User("12"))),
               new Meeting("meeting5",10,20, new User("5"), List.of(new User("82"))),
               new Meeting("meeting6",1,10, new User("6"), List.of(new User("29"))),
               new Meeting("meeting7",100,200, new User("90"), List.of(new User("21"))),
               new Meeting("meeting8",100,200, new User("90"), List.of(new User("21"))),
               new Meeting("meeting9",1000,2000, new User("90"), List.of(new User("21")))
        );

        ExecutorService executorService = Executors.newFixedThreadPool(3);
        List<Future> submittedTaskFutures = new ArrayList<>();
        for(Meeting meeting: meetings){
            submittedTaskFutures.add(executorService.submit(()->meetingScheduler.scheduleMeeting(meeting)));
        }
        room1.showMeetingHistory();
        room2.showMeetingHistory();

        // Added below code to wait for execution to be completed
        for(Future future: submittedTaskFutures) {
            try {
                future.get();
...
- Exception Handling when rooms are not available